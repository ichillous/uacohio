...
import { flagDuplicateLead } from "@/modules/staff/admissions/repository";
import { flagDuplicateSchema } from "@/modules/staff/admissions/stages";
import {
  parseJson,
  privateJson,
  requireSameOrigin,
...
export async function POST(request: Request, { params }: { params: Promise<{ leadId: string }> }) {
  try {
    ...
    const input = await parseJson(request, flagDuplicateSchema);
    const data = await flagDuplicateLead(await getD1Database(), leadId, input, {
