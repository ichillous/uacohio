...
import { transitionLead } from "@/modules/staff/admissions/repository";
import { transitionLeadSchema } from "@/modules/staff/admissions/stages";
import {
  parseJson,
  privateJson,
  requireSameOrigin,
...
export async function POST(request: Request, { params }: { params: Promise<{ leadId: string }> }) {
  try {
    ...
    const input = await parseJson(request, transitionLeadSchema);
    const data = await transitionLead(await getD1Database(), leadId, input, {
